...
__ = {
	...
	, dn : {
		...
		, _move : function( dnMove, dnTarget ) {
			if( __.dn._move_( dnMove, dnTarget, "_move" ) ) {
		...
		, move : function( dnMove, dnTarget ) {
			if( __.dn._move_( dnMove, dnTarget, "move" ) ) {
		...
		, move_ : function( dnMove, dnTarget ) {
			if( __.dn._move_( dnMove, dnTarget, "move_" ) ) {
				dnTarget.parentNode.insertBefore( dnMove, dnTarget.nextSibling );
			}
		}
		, _move_ : function( dnMove, dnTarget, sfn ) {
			if( dnMove instanceof HTMLCollection ) {
				__.each( dnMove, function( dn ) {
					setTimeout( function() {
						__.dn[ sfn ]( dn, dnTarget );
					}, 0 );
				} );
				return false;
			}	
...
// stubs for calling asynced methods as standalone
__.Async.hub = {
	  resolve : function( a ) { console.log( "[ok]", a ); }
	, reject : function( a ) { console.log( "[error]", a ); }
};
...
__.Async.Promise = function( args ) {
	...
	this.fnerr = ( args && args.fnerr ) ? args.fnerr : function( a, b ){ console.log( "ERR", a, b ) };
	this.fnend = ( args && args.fnend ) ? args.fnend : function( a ){ console.log( "OK", a ) };
	return this;
};
